    ...
    public class Mod : StardewModdingAPI.Mod
    {
        ...
        private void exportCommand(string str, string[] args)
        {
            ...
            if (args[0] == "all")
            {
                ...
                else if (args.Length > 1)
                {
                    ...
                    for (int i = 1; i < args.Length; ++i)
                    {
                        ...
                        {
                            case "tiles":
                                flags |= RenderFlags.Tiles;
                                break;
                            case "light":
                                flags |= RenderFlags.Lighting;
                                break;
                            case "npcs":
                                flags |= RenderFlags.Characters;
                                break;
                            case "player":
                                flags |= RenderFlags.Player;
                                break;
                            case "event":
                                flags |= RenderFlags.Event;
                                break;
                            case "weather":
                                flags |= RenderFlags.Weather;
                                break;
                            case "loc":
                                flags |= RenderFlags.Location;
                                break;
                        }
                    }
                }
            ...
            else if (args[0] == "list")
            {
                ...
                foreach (GameLocation loc in Game1.locations)
                {
                    if (loc.Equals(Game1.locations[0])) continue;
                    maps += ", " + loc.Name;
        ...
        private void checkRenderQueue(object sender, UpdateTickedEventArgs e)
        {
            ...
            if (renderQueue.TryDequeue(out RenderQueueEntry toRender)) export(toRender);
        }
        ...
        private void export(RenderQueueEntry render)
        {
            ...
            try
            {
                ...
                {
                    if (!loc.Equals(Game1.currentLocation)) loc.map.LoadTileSheets(display);
                ...
                if (loc.uniqueName.Value != null) name = loc.uniqueName.Value;
